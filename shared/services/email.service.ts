...
export async function sendBookingModificationEmail(
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: `${FROM_NAME} <${FROM_EMAIL}>`,
      to: customerEmail,
      subject: `Updates to your booking request`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: #f59e0b; padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
            <h1 style="color: white; margin: 0;">Booking Request Modified</h1>
          </div>
          
          <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px;">
            <p style="font-size: 16px;">Hi ${customerName},</p>
            
            <p style="font-size: 16px;">We've made some modifications to your booking request for <strong>${booking.boatName || 'your selected boat'}</strong>.</p>
            
            <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #f59e0b;">
              <h2 style="margin-top: 0; color: #f59e0b;">Modifications</h2>
              <p style="margin: 0;">${modifications}</p>
            </div>
            
            <p style="font-size: 16px;">Please review the changes above. If you'd like to proceed with these modifications, no further action is needed. If you have any concerns, please contact us.</p>
    ...
    console.log('Modification email sent successfully:', data?.id);
    return true;
  } catch (error) {
