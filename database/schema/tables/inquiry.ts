...
export const inquiry = pgTable(
  ...
  (table) => [index("inquiry_stage_idx").on(table.stage), index("inquiry_outcome_idx").on(table.outcome), index("inquiry_email_idx").on(table.email), index("inquiry_date_idx").on(table.createdAt)]
);
