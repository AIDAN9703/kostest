import {
  ...
  export const boats = pgTable("boat",{
      ...
      hasCalendar: boolean("has_calendar").default(false).notNull(),
  ...
  // Calendar credentials table - stores OAuth tokens for Google Calendar specifically
  // This is separate from the main OAuth login, as it needs different scopes
  export const calendarCredentials = pgTable("calendar_credentials", {
    id: uuid("id").defaultRandom().notNull().primaryKey(),
    userId: uuid("user_id").notNull().references(() => users.id),
    // Only store the Google-specific calendar tokens
    refreshToken: text("refresh_token").notNull(),
    accessToken: text("access_token"),
    tokenExpiry: timestamp("token_expiry", { mode: "date" }),
    // Add provider field to track which OAuth provider these credentials are for
    provider: text("provider").default("google").notNull(),
    // Scope information to know what these credentials grant access to
    scopes: text("scopes").array(),
    isActive: boolean("is_active").default(true).notNull(),
    createdAt: timestamp("created_at", { mode: "date", withTimezone: true }).defaultNow().notNull(),
    lastUsed: timestamp("last_used", { mode: "date", withTimezone: true }),
  }, (table) => [
    index("calendar_credentials_user_idx").on(table.userId)
  ]);
  
  // Boat calendars table - connects boats to Google Calendars
  export const boatCalendars = pgTable("boat_calendars", {
    id: uuid("id").defaultRandom().notNull().primaryKey(),
    boatId: uuid("boat_id").notNull().references(() => boats.id),
    ...
    index("boat_calendars_boat_idx").on(table.boatId),
    index("boat_calendars_owner_idx").on(table.ownerUserId),
    // Add standard index
    index("boat_calendars_boat_owner_idx").on(table.boatId, table.ownerUserId)
  ]);
  
  // Calendar sync logs table - tracks sync operations
  export const calendarSyncLogs = pgTable("calendar_sync_logs", {
    id: uuid("id").defaultRandom().notNull().primaryKey(),
    boatCalendarId: uuid("boat_calendar_id").notNull().references(() => boatCalendars.id),
    status: calendarSyncStatusEnum("status").notNull(),
    message: text("message"),
    eventsProcessed: integer("events_processed"),
    createdAt: timestamp("created_at", { mode: "date", withTimezone: true }).defaultNow().notNull(),
  }, (table) => [
    index("calendar_sync_logs_boat_calendar_idx").on(table.boatCalendarId),
    index("calendar_sync_logs_status_idx").on(table.status)
  ]);
  
  // Boat availability table - stores all availability events
  export const boatAvailability = pgTable("boat_availability", {
    id: uuid("id").defaultRandom().notNull().primaryKey(),
    boatId: uuid("boat_id").notNull().references(() => boats.id),
    eventId: text("event_id"), // Google Calendar event ID if from calendar
    ...
    index("boat_availability_boat_idx").on(table.boatId),
    index("boat_availability_booking_idx").on(table.bookingId),
    index("boat_availability_event_idx").on(table.eventId),
    index("boat_availability_time_idx").on(table.startTime, table.endTime)
  ]);
  
  // Availability holds table - for temporary reservation of time slots
  export const availabilityHolds = pgTable("availability_holds", {
    ...
    index("availability_holds_boat_idx").on(table.boatId),
    index("availability_holds_user_idx").on(table.userId),
    index("availability_holds_expires_idx").on(table.expiresAt)
  ]);