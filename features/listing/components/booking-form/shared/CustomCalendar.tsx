...
export function CustomCalendar({
  ...
  const availabilityMap = useMemo(
    () => new Map(calendarDays.map((d) => [d.date.split("T")[0], d])),
    [calendarDays]
  );
    ...
    (async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/boats/${boatId}/calendar?month=${monthParam}`
        );
        if (!response.ok)
  ...
  const renderDay = useCallback(
    (day: Date) => {
      ...
      const isDisabled =
        isPast || info?.status === "booked" || info?.status === "blocked";
      let underline = "";
  ...
  return (
    <div
      className="p-2 border border-gray-100 rounded-2xl max-w-[92vw] w-[min(380px,92vw)] select-none"
    ...
    >
      <div className="flex items-center justify-between px-2 mb-2">
        <button
          type="button"
          onClick={goPrev}
          className="h-7 w-7 rounded-full text-gray-600 hover:bg-gray-50 focus:outline-hidden active:scale-95"
        >
          ‹
        </button>
      ...
      <div className="grid grid-cols-7 gap-1 px-2 text-[0.7rem] text-muted-foreground">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
          <div key={d} className="text-center font-medium w-9 sm:w-10 mx-auto">
            {d}
          </div>
        ))}
      </div>
            ...
            <div key={i} className="grid grid-cols-7 gap-1 px-2 select-none">
              {r.map((day, j) =>
                day ? (
                  ...
                  <div key={`empty-${i}-${j}`} className="h-9 sm:h-10" />
                )
              )}
            </div>
          ));
