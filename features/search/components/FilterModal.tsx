...
import { 
  Calendar as CalendarIcon, Loader2, X, Filter, Check, ChevronDown
} from "lucide-react";
...
export default function FilterModal({ isOpen, onClose }: FilterModalProps) {
  ...
  const [guests, setGuests] = useState(getInitialNumberParam('passengers', DEFAULT_FILTERS.guests));
  const [cabins, setCabins] = useState(getInitialNumberParam('cabins', DEFAULT_FILTERS.cabins));
  const [bathrooms, setBathrooms] = useState(getInitialNumberParam('bathrooms', DEFAULT_FILTERS.bathrooms));
  ...
  // Categories data
  const [categories, setCategories] = useState<{category: string; count: number; displayName: string}[]>([]);
  ...
  const activeFilterCount = useCallback(() => {
    ...
    if (priceRange[0] > DEFAULT_FILTERS.priceRange[0] || 
        priceRange[1] < DEFAULT_FILTERS.priceRange[1]) count++;
    if (lengthRange[0] > DEFAULT_FILTERS.lengthRange[0] || 
        lengthRange[1] < DEFAULT_FILTERS.lengthRange[1]) count++;
    if (yearBuilt[0] > DEFAULT_FILTERS.yearBuilt[0] || 
        yearBuilt[1] < DEFAULT_FILTERS.yearBuilt[1]) count++;
    ...
    date, priceRange, lengthRange, yearBuilt, 
    guests, cabins, bathrooms, selectedCategories, selectedFeatures
  ]);
          ...
          <div className="filter-section">
            ...
            >
              <h3 className="text-base font-medium flex items-center">
                Date
                {date && <Badge className="ml-2 bg-blue-100 text-blue-800 hover:bg-blue-200">Selected</Badge>}
              </h3>
                  ...
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        date && "text-primary border-primary border-solid font-medium"
                      )}
                    >
            ...
            >
              <h3 className="text-base font-medium flex items-center">
                ...
                {(priceRange[0] > DEFAULT_FILTERS.priceRange[0] || 
                  ...
                  <Badge className="ml-2 bg-blue-100 text-blue-800 hover:bg-blue-200">Custom</Badge>
                )}
              </h3>
              <ChevronDown className={cn(
                "h-5 w-5 transition-transform", 
            ...
            {activeSection === 'length' && (
              <div className="mt-3 mb-1 py-2">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="bg-gray-50 px-3 py-2 rounded-md border text-sm w-24 text-center">
                    {lengthRange[1] === 100 ? '100+ ft' : `${lengthRange[1]} ft`}
                  </div>
                </div>
                <Slider
                  value={lengthRange}
                ...
                <div className="grid grid-cols-3 gap-2 mt-3">
                  <Button 
                    ...
                    className={cn(
                      "h-10",
                      lengthRange[0] === 0 && lengthRange[1] === 30 ? "bg-primary/10 border-primary" : ""
                    )}
                  >
                    Up to 30ft
                  </Button>
                  <Button 
                    ...
                    className={cn(
                      "h-10",
                      lengthRange[0] === 30 && lengthRange[1] === 60 ? "bg-primary/10 border-primary" : ""
                    )}
                  >
                    30-60ft
                  </Button>
                  <Button 
                    ...
                    className={cn(
                      "h-10",
                      lengthRange[0] === 60 && lengthRange[1] === 100 ? "bg-primary/10 border-primary" : ""
                    )}
                  >
                    60ft+
                  </Button>
                      ...
                      {categories.map((cat) => (
                        ...
                        >
                          <Checkbox 
                            ...
                            className={cn(
                              "mr-3",
                              selectedCategories.includes(cat.category) ? "border-primary" : ""
                            )}
                          />
                          <div className="flex-1">
                            <label htmlFor={`cat-${cat.category}`} className="cursor-pointer w-full font-medium">
                              {cat.displayName}
                            </label>
                            <p className="text-xs text-gray-500">{cat.count} boats</p>
                          </div>
                          {selectedCategories.includes(cat.category) && (
