...
interface AdminBoatCalendarManagerProps {
  boatId: string;
  boatName: string;
...
export default function AdminBoatCalendarManager({
  boatId,
  boatName,
  calendars,
  onCalendarsChange,
}: AdminBoatCalendarManagerProps) {
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [syncingCalendars, setSyncingCalendars] = useState<Set<string>>(
    new Set(),
  );
  ...
  const handleSyncCalendar = async (calendarId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/calendar-sync?boatCalendarId=${calendarId}`,
        {
          method: "POST",
        },
      );
  ...
  const handleRemoveCalendar = async (calendarId: string) => {
    try {
      const response = await fetch(
        `/api/admin/boats/${boatId}/calendars/${calendarId}`,
        {
          method: "DELETE",
        },
      );
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Get this from Google Calendar → Settings → Share → Secret
      ...
      <CardContent>
        {calendars.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm">
              Add boat owner's Google Calendar to sync availability
            </p>
          </div>
        ) : (
          <div className="space-y-4">
