...
import { Eye, MoreVertical, Pencil, UsersRound } from "lucide-react";
...
const columnHelper = createColumnHelper<CrewProfileAdminRow>();

interface AdminCrewProfilesTableProps {
...
export function AdminCrewProfilesTable({ rows, loading }: AdminCrewProfilesTableProps) {
  const columns = useMemo<ColumnDef<CrewProfileAdminRow, any>[]>(
    () => [
      ...
      columnHelper.accessor("adminNotes", {
        ...
        cell: (info) => {
          ...
          return (
            <span className="text-sm text-muted-foreground" title={v || undefined}>
              {short || "—"}
            </span>
          );
        },
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <h3 className="mb-2 text-lg font-medium text-foreground">No crew profiles</h3>
          <p className="text-sm text-muted-foreground">
            Enable the crew profile on a user, or adjust filters.
          </p>
        </div>
      </div>
    );
