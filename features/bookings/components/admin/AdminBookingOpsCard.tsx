...
import type { BookingOpsData } from "@/features/bookings/booking-ops.service";
...
export function AdminBookingOpsCard({ bookingId, ops }: AdminBookingOpsCardProps) {
  ...
  const [durationHours, setDurationHours] = useState(ops?.durationHours ?? "");
  ...
  const [balanceClientCents, setBalanceClientCents] = useState(
    centsToDisplay(ops?.balanceClientCents)
  );
  ...
  const [commissionCents, setCommissionCents] = useState(
    centsToDisplay(ops?.commissionCents)
  );
