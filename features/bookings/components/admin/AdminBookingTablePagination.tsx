...
export function AdminBookingTablePagination({
  totalCount,
  totalPages,
  page,
  limit,
}: AdminBookingTablePaginationProps) {
  const searchParams = useSearchParams();

  if (totalPages <= 1) return null;

  const basePath = "/admin/bookings";

  const makeHref = (newPage: number) => {
    const query = serializeBookingParams(searchParams, { page: newPage });
