...
import { CheckCircle, Calendar, Home, Printer, Download, Share2, Clock } from "lucide-react";
...
export default function BookingSuccessClient({ user }: BookingSuccessClientProps) {
  ...
  const formattedDate = startDateTime ? format(new Date(startDateTime), "EEEE, MMM d, yyyy 'at' h:mma") : null;
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-80px)]">
      <div className={`max-w-7xl 2xl:max-w-8xl mx-auto w-full flex-1 px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 py-6 sm:py-8 lg:py-10 xl:py-12 2xl:py-16 transition-all duration-700 ${showAnimation ? 'opacity-100' : 'opacity-0'}`}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-8 xl:mb-10 gap-4">
          ...
          <div className={`px-3 py-1.5 rounded-full text-xs font-medium self-start sm:self-auto ${isInstantBook ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
            {isInstantBook ? 'Confirmed' : 'Under Review'}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 2xl:grid-cols-4 gap-6 lg:gap-8 xl:gap-12 2xl:gap-16 items-start">
          <div className="w-full xl:col-span-2 2xl:col-span-3 space-y-4 sm:space-y-6 xl:space-y-8">
            ...
            <div className="bg-white">
              <div className="flex gap-3 sm:gap-4 xl:gap-6">
                ...
                <div className="flex-1 min-w-0">
                  <h2 className="text-base sm:text-lg xl:text-xl font-semibold text-gray-900 truncate">{boatName || 'Boat Booking'}</h2>
                  <p className="text-sm xl:text-base text-gray-600 mt-1 break-words">
                    {formattedDate || '—'}{hours ? ` • ${hours} hours` : ''}
                  </p>
                </div>
              </div>
            </div>
            ...
            {/* Booking Status */}
            <div className="bg-white">
              <h3 className="text-sm xl:text-base font-semibold text-gray-900 mb-3 xl:mb-4">Booking Status</h3>
            ...
            {/* Next Steps */}
            <div className="bg-white">
              <h3 className="text-sm xl:text-base font-semibold text-gray-900 mb-3 xl:mb-4">Next Steps</h3>
          ...
          <div className="space-y-4 sm:space-y-6 xl:space-y-8 xl:sticky xl:top-6 2xl:top-8 self-start">
            {/* Price Breakdown */}
            <div className="bg-white">
              <h3 className="text-sm xl:text-base font-semibold text-gray-900 mb-4 xl:mb-6">Price Breakdown</h3>
              <div className="bg-gray-50 rounded-lg p-4 xl:p-5">
                <div className="space-y-3 xl:space-y-4 text-sm xl:text-base">
                  <div className="flex justify-between text-gray-700">
                    <span>Base price</span>
                    <span className="font-medium">{basePrice ? formatCurrency(parseFloat(basePrice)) : '—'}</span>
                  </div>
                  {cleaningFee && (
                    <div className="flex justify-between text-gray-700">
                      <span>Cleaning fee</span>
                      <span className="font-medium">{formatCurrency(parseFloat(cleaningFee))}</span>
                    </div>
                  )}
                  {serviceFee && (
                    <div className="flex justify-between text-gray-700">
                      <span>Service fee</span>
                      <span className="font-medium">{formatCurrency(parseFloat(serviceFee))}</span>
                    </div>
                  )}
                  <div className="border-t border-gray-300 pt-3 xl:pt-4">
                    <div className="flex justify-between text-gray-900 font-bold text-base xl:text-lg">
                      <span>Total</span>
                      <span className="text-primary">{totalAmount ? formatCurrency(parseFloat(totalAmount)) : '—'}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3 xl:space-y-4">
              ...
              <div className="border-t border-gray-200 pt-3 xl:pt-4">
                <div className="text-xs font-medium text-gray-500 mb-2 uppercase tracking-wide">Save & Share</div>
                <div className="grid grid-cols-3 gap-2">
                  ...
                  >
                    ...
                    <span className="text-xs font-medium text-gray-600 group-hover:text-gray-900">Print</span>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <span className="text-xs font-medium text-gray-600 group-hover:text-gray-900">PDF</span>
                  </button>
                  <button
                    onClick={handleShare}
                    className="flex flex-col items-center justify-center p-3 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors touch-manipulation group"
                  >
                    ...
                    <span className="text-xs font-medium text-gray-600 group-hover:text-gray-900">Share</span>
                  </button>
                </div>
              </div>
            </div>
