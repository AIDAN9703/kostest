...
export class BookingNotesService {
  ...
  async markAsContacted(bookingId: string, adminUserId: string, details?: string): Promise<void> {
  ...
  async addFollowUp(bookingId: string, adminUserId: string, content: string): Promise<void> {
    await bookingEventsService.logNote({
      bookingId,
      actorId: adminUserId,
      content,
  ...
  async addIssue(bookingId: string, adminUserId: string, content: string): Promise<void> {
    await bookingEventsService.logNote({
      bookingId,
      actorId: adminUserId,
      content,
  ...
  async hasBeenContacted(bookingId: string): Promise<boolean> {
    ...
    const [note] = await db
      ...
      .from(bookingAdminNotes)
      .where(
        and(eq(bookingAdminNotes.bookingId, bookingId), eq(bookingAdminNotes.noteType, "CONTACTED"))
      )
      .limit(1);
    return !!note;
  }
