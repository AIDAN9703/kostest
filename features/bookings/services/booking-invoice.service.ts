...
export async function createDraftBookingInvoice(
  ...
  const isDeposit = firstDraftBooking.paymentType === "DEPOSIT_ONLY";
  ...
  for (const b of draftBookings) {
    ...
    const fullCents = pricing ? Number(pricing.totalAmountCents) : 0;
    const depositCents = pricing?.depositAmountCents
      ? Number(pricing.depositAmountCents)
      : 0;
    const chargeCents =
      isDeposit && depositCents > 0 ? depositCents : fullCents;
    totalCents += chargeCents;

    const description = isDeposit && depositCents > 0
      ? `${boatName} — Deposit`
      : `${boatName} — Charter`;
  ...
  const paymentType = isDeposit ? "DEPOSIT" : "FULL_PAYMENT";
