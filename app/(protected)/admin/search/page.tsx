...
export default async function SearchPage({
        ...
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-gray-500 max-w-md mx-auto">
              Enter at least 2 characters to search across users, boats, and
              bookings.
            </p>
          </CardContent>
        </Card>
  ...
  if (boatResults.length > 0) {
    ...
    const tiersByBoatId = pricingTiersResults.reduce(
      (acc, tier) => {
  ...
  const totalResults =
    userResults.length + boatResults.length + bookingResults.length;
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href={`/admin/search?q=${encodeURIComponent(query)}&type=all`}>
            <Button
              variant={type === "all" || !type ? "default" : "outline"}
              size="sm"
            >
              All
            </Button>
          ...
          <Link
            href={`/admin/search?q=${encodeURIComponent(query)}&type=users`}
          >
            <Button
          ...
          <Link
            href={`/admin/search?q=${encodeURIComponent(query)}&type=boats`}
          >
            <Button
              variant={type === "boats" ? "default" : "outline"}
              size="sm"
            >
              Boats
            </Button>
          ...
          <Link
            href={`/admin/search?q=${encodeURIComponent(query)}&type=bookings`}
          >
            <Button
              variant={type === "bookings" ? "default" : "outline"}
              size="sm"
            >
              Bookings
            </Button>
                    ...
                    {boatsWithPricingTiers.map((boat) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-medium">
                          ...
                          {getBoatDefaultHours(boat)}
                        </td>
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex h-6 w-6 items-center justify-center rounded-full ${boat.active ? "bg-green-500" : "bg-gray-300"}`}
                          >
      ...
      {totalResults === 0 && (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-gray-500 max-w-md mx-auto">
              We couldn't find any results matching "{query}". Try using
