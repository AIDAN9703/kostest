...
export async function generateMetadata({ params }: UserDetailPageProps): Promise<Metadata> {
  ...
  const user = await userService.getUserForAdmin(resolvedParams.id).catch(() => null);
...
async function UserProfile({ userId }: { userId: string }) {
  // Use getUserForAdmin for optimized query with owned boats and recent bookings
  const user = await userService.getUserForAdmin(userId).catch(() => null);
