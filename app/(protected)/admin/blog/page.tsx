...
import { DataTablePagination } from "@/shared/admin/components/DataTablePagination";
...
export default async function BlogPage({ searchParams }: BlogPageProps) {
  ...
  const currentPage = resolvedParams.page
    ? Math.max(1, parseInt(resolvedParams.page))
    : 1;
  const limit = resolvedParams.limit
    ? Math.max(10, Math.min(100, parseInt(resolvedParams.limit)))
    : ITEMS_PER_PAGE;
