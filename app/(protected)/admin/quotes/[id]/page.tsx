...
const quote = {
  ...
  details: {
    ...
    specialRequests:
      "Customer requested ice and drinks to be available. Will bring own food.",
...
export default async function QuoteDetailsPage({
  ...
  const daysUntilExpiry = Math.ceil(
    (new Date(quote.dates.expiresAt).getTime() - new Date().getTime()) /
      (1000 * 60 * 60 * 24)
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Created on {new Date(quote.dates.createdAt).toLocaleDateString()} at{" "}
            {new Date(quote.dates.createdAt).toLocaleTimeString()}
          </p>
        </div>
      ...
      {/* Status Banner */}
      <Card
        className={`${isExpired ? "bg-red-50 border-red-200" : "bg-amber-50 border-amber-200"}`}
      >
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <p className="mt-1">
                    {new Date(quote.details.date).toLocaleDateString()}
                  </p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-gray-500">Time</h3>
                  <p className="mt-1">
                    {quote.details.startTime} - {quote.details.endTime} (
                    {quote.details.numberOfHours} hours)
                  </p>
                </div>
                <div>
            ...
            <CardContent>
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <Link
                    href={`/admin/users/${quote.customer.id}`}
            ...
            <CardContent>
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <Link
                    href={`/admin/boats/${quote.boat.id}`}
                    className="text-sm text-primary hover:underline"
                  >
                    View Boat
                  </Link>
              ...
              <div className="space-y-2 mb-4">
                <div>
                  ...
                  <p className="text-sm">
                    {formatBoatCategory(quote.boat.category)}
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-medium">
                      {quote.boat.owner.name}
                    </p>
                    <p className="text-xs text-gray-500">
                      {quote.boat.owner.email}
                    </p>
                  </div>
                </div>
                <div className="mt-2">
                  <Link
                    href={`/admin/users/${quote.boat.owner.id}`}
              ...
              <div className="space-y-4">
                <div className="flex">
                  ...
                  <div className="pb-4">
                    ...
                    <p className="text-sm text-gray-500">
                      {new Date(quote.dates.createdAt).toLocaleDateString()} at{" "}
                      {new Date(quote.dates.createdAt).toLocaleTimeString()}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex">
                  ...
                  <div className="pb-4">
                    ...
                    <p className="text-sm text-gray-500">
                      {new Date(quote.dates.sentAt).toLocaleDateString()} at{" "}
                      {new Date(quote.dates.sentAt).toLocaleTimeString()}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {new Date(quote.dates.expiresAt).toLocaleDateString()} at{" "}
                      {new Date(quote.dates.expiresAt).toLocaleTimeString()}
                    </p>
                  </div>
                </div>
              </div>
