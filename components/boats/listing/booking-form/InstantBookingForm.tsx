...
export default function InstantBookingForm({ boat, user }: InstantBookingFormProps) {
  ...
  const handleSubmit = async (data: BookingRequest) => {
    ...
    try {
      ...
      console.error("Error submitting instant booking:", error);
      toast({
    ...
    <div className="p-4">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
          ...
          <Button 
            type="submit" 
            className="w-full h-11 font-semibold text-white transition-all duration-200 text-sm shadow-sm hover:shadow-md rounded-lg border-0 bg-coral-500 hover:bg-coral-600"
          ...
          >
            {isSubmitting ? (
              ...
              <div className="flex items-center gap-2">
                ...
                {isAuthenticated ? "Book & Pay Now" : "Continue to Book"}
              </div>
            )}
          </Button>

          {/* Instant booking note */}
          <div className="bg-coral-50 border border-coral-200 rounded-lg p-2">
            <div className="text-sm text-coral-700 text-center">
