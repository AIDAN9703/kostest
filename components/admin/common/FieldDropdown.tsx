...
import { updateField } from "@/lib/actions/admin/universal-updates";
import { ADMIN_FIELD_DROPDOWN_OPTIONS, type FieldOption } from "@/lib/constants/admin-field-dropdown-constants";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function FieldDropdown({
  ...
  const handleValueChange = async (newValue: string | boolean) => {
    ...
    startTransition(async () => {
      try {
        const result = await updateField(entity, id, field, newValue);
